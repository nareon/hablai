...
# результат и чекпоинт — на sda2
OUTPUT_INDEX = Path("/media/ol/SSD2T_Photo/hablai/corpus/jsonl/phrase_index_llm_filtered.jsonl")
...
BATCH_SIZE = 32  # сколько фраз отправляем в LLM за один запрос
...
def load_checkpoint() -> int:
    """
    Вернуть номер последней обработанной строки исходного phrase_index.jsonl.
...
# базовый алфавит для "похожести на испанский"
...
def simple_prefilter(rec: dict) -> bool:
    ...
    if phrase.startswith(("¡", "¿")) and not has_common_verb():
        # что-то вроде "¡¡¡Naughty America", "¡Comparte!", "¡¡Error de acceso..."
        return False
...
- ЕСЛИ ТЫ СОМНЕВАЕШЬСЯ, лучше ставь keep = true.
...
def main():
    ...
    kept_total = 0  # сколько фраз в итоге приняли
    ...
    with PHRASE_INDEX.open("r", encoding="utf-8") as inp, \
        ...
        for line_no, line in enumerate(tqdm(inp, desc="scanning phrase_index")):
            ...
            if len(batch_for_llm) >= BATCH_SIZE:
                ...
                if (line_no // BATCH_SIZE) % 500 == 0:
                    kept = sum(1 for d in decisions.values() if d["keep"])
                    print(f"[info] batch #{line_no // BATCH_SIZE}: kept {kept} of {len(batch_for_llm)}")
    ...
    print("Total kept phrases:", kept_total)
